# 入力/コントロールマニフェスト

*Created: 2026-04-27*
*Status: Draft*

---

## 1. Overview

入力/コントロールマニフェストは、VectorFlow における全プレイヤー操作の定義書です。キーボード/マウスをプライマリとし、ゲームパッドをセカンダリとする2系統の入力を管理します。アクションは「コアアクション（移動・ジャンプ・しゃがみ・ワイヤー・攻撃）」「スキルスロット（将来拡張）」「アルティメットスロット（将来拡張）」の3カテゴリに分類します。新アクションはスロットにバインドを追加するだけで既存定義を変更せず拡張できる設計とします。全操作系システムはこのマニフェストを入力の唯一の参照源として扱います。

---

## 2. Player Fantasy

プレイヤーは「考えるより先に体が動く」感覚を目指す。マウスで狙った瞬間にワイヤーが飛び、右クリックを離せばそこへ引き寄せられ、左クリックを握り込むほど刃に力がこもる——入力と結果の間に遅延や曖昧さはない。スキルとアルティメットは既存の操作リズムを壊さず「いつものボタン配置の延長」として手に馴染み、習得後は意識せず使えるようになる。

---

## 3. Detailed Rules

### アクションカテゴリ

| カテゴリ | 説明 | 拡張性 |
|---------|------|--------|
| **Core** | 常時使用するコアアクション | 固定 |
| **Skill** | スキルスロット（現在は空、将来追加） | スロット追加で拡張 |
| **Ultimate** | アルティメットスロット（現在は空、将来追加） | スロット追加で拡張 |

---

### Core アクション定義

| アクション名 | 入力タイプ | KB/M | ゲームパッド | 備考 |
|-------------|-----------|------|------------|------|
| `move_left` | Hold | A | 左スティック左 | |
| `move_right` | Hold | D | 左スティック右 | |
| `jump` | Pressed | Space | A（Cross） | 壁蹴りジャンプも同キー |
| `crouch` | Hold | S | B（Circle） | 押下中だけ有効 |
| `wire_shoot` | Pressed | 右クリック | 右トリガー（RT） | 照準はマウス座標 / 右スティック方向 |
| `attack` | Released | 左クリック | X（Square） | 長押し時間で通常/溜めを判定 |

---

### Skill / Ultimate スロット（将来拡張用）

| スロット | KB/M | ゲームパッド | 現在の割り当て |
|---------|------|------------|-------------|
| `skill_1` | Q | LB（L1） | 未割り当て |
| `skill_2` | E | RB（R1） | 未割り当て |
| `ultimate` | R | LT+RT（L2+R2） | 未割り当て |

---

### 入力タイプ定義

| タイプ | Godot 対応メソッド | 説明 |
|--------|-------------------|------|
| Pressed | `Input.is_action_just_pressed()` | 押した瞬間の1フレームのみ |
| Hold | `Input.is_action_pressed()` | 押し続けている間ずっと |
| Released | `Input.is_action_just_released()` | 離した瞬間の1フレームのみ |

---

### 攻撃入力の判定フロー

```
左クリック押下開始 → charge_timer カウント開始
  ├─ リリース時 charge_timer < 0.4s  → 通常攻撃を発動
  └─ リリース時 charge_timer >= 0.4s → 溜め攻撃を発動
```

溜め中も他アクション（`wire_shoot` 等）は独立して受け付ける。

---

## 4. Formulas

### F-1: 溜め攻撃判定

```
is_charge = (charge_timer >= CHARGE_THRESHOLD)

変数:
  charge_timer     : 左クリック押下からの経過秒数 [s]
  CHARGE_THRESHOLD : 溜め判定の閾値（デフォルト 0.4s）

例:
  charge_timer = 0.39s → is_charge = false → 通常攻撃
  charge_timer = 0.40s → is_charge = true  → 溜め攻撃
```

### F-2: マウス照準ベクトル（KB/M）

```
wire_direction = (mouse_world_pos - player_pos).normalized()

変数:
  mouse_world_pos : get_global_mouse_position() で取得したワールド座標
  player_pos      : プレイヤーのグローバル座標

結果: 長さ1の単位ベクトル。ワイヤーシステムに渡す。
```

### F-3: スティック照準ベクトル（ゲームパッド）

```
stick_raw = Input.get_vector("move_left", "move_right", "move_up", "move_down")
※ 照準専用スティックは右スティック（将来実装）

if stick_raw.length() >= STICK_DEADZONE:
    wire_direction = stick_raw.normalized()
else:
    wire_direction = last_wire_direction  # 前回の方向を維持

変数:
  STICK_DEADZONE       : スティックの無効帯（デフォルト 0.2）
  last_wire_direction  : 直前に確定した照準方向（初期値: Vector2.RIGHT）
```

---

## 5. Edge Cases

| # | 状況 | 処理 |
|---|------|------|
| E-1 | 溜め中にプレイヤーが死亡した場合 | `charge_timer` をリセット。攻撃は発動しない |
| E-2 | 溜め中に `wire_shoot` を入力した場合 | ワイヤーは即時発動。溜めはそのままカウント継続 |
| E-3 | `jump` と `crouch` を同時入力した場合 | `jump` を優先。`crouch` は無視 |
| E-4 | KB/M とゲームパッドを同時に操作した場合 | 最後に入力があったデバイスを優先。切り替えは自動検出 |
| E-5 | スティックの入力値がちょうど `STICK_DEADZONE`（0.2）の場合 | `>= STICK_DEADZONE` のため有効入力として扱う |
| E-6 | ゲームパッド照準でスティックが無入力の場合 | `last_wire_direction` を維持。ゼロベクトルにはならない |
| E-7 | ポーズ中/メニュー表示中にゲームプレイ入力が来た場合 | 全 Core・Skill・Ultimate アクションを無効化。UI 専用入力のみ受け付ける |
| E-8 | 未割り当てスキルスロット（`skill_1` 等）を押した場合 | 何も起きない。エラーは出さない |

---

## 6. Dependencies

### このシステムが依存するもの

なし（Foundation レイヤー — 全システムの起点）

### このシステムに依存するシステム

| システム | 依存内容 |
|---------|---------|
| プレイヤーシステム (#4) | `move_left` / `move_right` / `jump` / `crouch` の入力読み取り |
| ワイヤーシステム (#6) | `wire_shoot` の入力検知・照準ベクトル取得 |
| 戦闘システム (#7) | `attack` の押下/リリース・`charge_timer` の値 |
| スキルシステム（将来） | `skill_1` / `skill_2` の入力検知 |
| アルティメットシステム（将来） | `ultimate` の入力検知 |

### 実装上の参照

Godot の `InputMap` をバインディングの実体として使用する。このマニフェストに定義されたアクション名と `InputMap` のアクション名を 1:1 で対応させる。

---

## 7. Tuning Knobs

| 定数名 | デフォルト | 安全範囲 | 影響するゲームプレイ |
|--------|-----------|---------|-------------------|
| `CHARGE_THRESHOLD` | 0.4s | 0.2s〜0.8s | 溜め攻撃の発動しやすさ。小さいほどカジュアル、大きいほど慎重な操作が必要 |
| `STICK_DEADZONE` | 0.2 | 0.1〜0.35 | ゲームパッドの誤入力耐性。小さいほど敏感、大きいほど不感帯が広がる |
| `INPUT_BUFFER_FRAMES` | 6 frames | 0〜12 frames | ジャンプなど Pressed 系アクションの先行入力受付フレーム数。大きいほど操作が鷹揚になる |

> 安全範囲外の値は動作保証外。`CHARGE_THRESHOLD` を 0.1s 未満にすると通常攻撃がほぼ出なくなる。

---

## 8. Acceptance Criteria

| # | テスト内容 | 合格条件 |
|---|-----------|---------|
| AC-1 | 左クリックを 0.39s 押してリリース | 通常攻撃が発動する |
| AC-2 | 左クリックを 0.40s 押してリリース | 溜め攻撃が発動する |
| AC-3 | 溜め中（0.4s未満）に右クリック | ワイヤーが発動し、溜めカウントは継続する |
| AC-4 | Space と S を同時押し | ジャンプのみ発動し、しゃがみは発動しない |
| AC-5 | ゲームパッドスティックを倒し量 0.19 で操作 | キャラクターは動かない（デッドゾーン内） |
| AC-6 | ゲームパッドスティックを倒し量 0.20 で操作 | キャラクターが動く |
| AC-7 | KB/M 操作中にゲームパッドのスティックを入力 | 入力デバイスがゲームパッドへ自動切り替えされる |
| AC-8 | ポーズ中に攻撃・ワイヤー・移動を入力 | いずれも反応しない |
| AC-9 | `skill_1`（Q）を押す（未割り当て状態） | エラーなし、何も起きない |
| AC-10 | `InputMap` のアクション一覧を確認 | マニフェスト定義の全アクション名が存在する |
