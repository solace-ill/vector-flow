...
| # | 状況 | 処理 |
|---|------|------|
| E-1 | 溜め中にプレイヤーが死亡した場合 | `charge_timer` をリセット。攻撃は発動しない |
| E-2 | 溜め中に `wire_shoot` を入力した場合 | ワイヤーは即時発動。溜めはそのままカウント継続 |
| E-3 | `jump` と `crouch` を同時入力した場合 | `jump` を優先。`crouch` は無視 |
| E-4 | KB/M とゲームパッドを同時に操作した場合 | 最後に入力があったデバイスを優先。切り替えは自動検出 |
| E-5 | スティックの入力値がちょうど `STICK_DEADZONE`（0.2）の場合 | `>= STICK_DEADZONE` のため有効入力として扱う |
| E-6 | ゲームパッド照準でスティックが無入力の場合 | `last_wire_direction` を維持。ゼロベクトルにはならない |
| E-7 | ポーズ中/メニュー表示中にゲームプレイ入力が来た場合 | 全 Core・Skill・Ultimate アクションを無効化。UI 専用入力のみ受け付ける |
| E-8 | 未割り当てスキルスロット（`skill_1` 等）を押した場合 | 何も起きない。エラーは出さない |
...
なし（Foundation レイヤー — 全システムの起点）

### このシステムに依存するシステム

| システム | 依存内容 |
|---------|---------|
| プレイヤーシステム (#4) | `move_left` / `move_right` / `jump` / `crouch` の入力読み取り |
| ワイヤーシステム (#6) | `wire_shoot` の入力検知・照準ベクトル取得 |
| 戦闘システム (#7) | `attack` の押下/リリース・`charge_timer` の値 |
| スキルシステム（将来） | `skill_1` / `skill_2` の入力検知 |
| アルティメットシステム（将来） | `ultimate` の入力検知 |

### 実装上の参照

Godot の `InputMap` をバインディングの実体として使用する。このマニフェストに定義されたアクション名と `InputMap` のアクション名を 1:1 で対応させる。
...
|--------|-----------|---------|-------------------|
| `CHARGE_THRESHOLD` | 0.4s | 0.2s〜0.8s | 溜め攻撃の発動しやすさ。小さいほどカジュアル、大きいほど慎重な操作が必要 |
| `STICK_DEADZONE` | 0.2 | 0.1〜0.35 | ゲームパッドの誤入力耐性。小さいほど敏感、大きいほど不感帯が広がる |
| `INPUT_BUFFER_FRAMES` | 6 frames | 0〜12 frames | ジャンプなど Pressed 系アクションの先行入力受付フレーム数。大きいほど操作が鷹揚になる |

> 安全範囲外の値は動作保証外。`CHARGE_THRESHOLD` を 0.1s 未満にすると通常攻撃がほぼ出なくなる。
